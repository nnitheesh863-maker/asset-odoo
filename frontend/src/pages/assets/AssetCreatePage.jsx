...
export default function AssetCreatePage() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const res = await assetService.create(formData);
      toast.success('Asset created successfully');
      navigate(`/assets/${res.data?.id || res.id}`);
    } catch (err) {
  ...
  if (loadingFilters) return <LoadingSpinner centered />;

  return (
    <div>
      <PageHeader title="Register Asset" subtitle="Add a new asset to the system" icon={Package}>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="max-w-3xl">
        <div className="space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <button type="button" onClick={() => navigate('/assets')} className="btn btn-secondary">
              Cancel
            </button>
          </div>
        </div>
